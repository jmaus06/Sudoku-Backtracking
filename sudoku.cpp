...
int main() {
	...
	for (tempvar = 1; tempvar <= 81; ++tempvar) //set nochangeit to false to start
		noChangeIt[tempvar] = false;

	for (int temp1 = 1; temp1 <= 9; ++temp1) {
		for (int temp2 = 1; temp2 <= 9; ++temp2) //assign everything to 0
...
void solveIt(int currentCube, int num[][10], int ro, int co, int bo, bool solvedIt, bool noChange[], int& countIt) {
	do {	
		...
		if (checkRow(ro, co, bo, num) && checkCol(ro, co, bo, num)  && checkBox(ro,co,bo,num)) { //if the cube meets all requirements, go forward
			//&& checkBox(ro, co, bo, num)
				...
				} while (noChange[currentCube] == true); //repeat until the cube is NOT a permament spot
				...
				if ((num[ro][co] < 9) && (noChange[currentCube] == false)) {  //if it is NOT at 9 already
					...
					do {
						...
						while (keepgoingback == true) { //go back until the number is less than 9
							...
							if (onRowBoxBegin(ro, co, currentCube)) { //at beginning of row and box
								...
								--bo;
								--currentCube;
								if ((noChange[currentCube] == false) && (num[ro][co] < 9)) { //start new space at next one
									++num[ro][co];
									keepgoingback = false;
								}
	...
	cout << endl;
	for (int tempx = 1; tempx <= 9; ++tempx) {  //show when solved
...
bool checkRow(int row, int col, int box, int num[][10]) { //see if number already exists in row
...
bool checkCol(int row, int col, int box, int num[][10]) { //see if number already exists in column
...
bool checkBox(int row, int col, int box, int num[][10]) {
	...
	switch (box) { //checks if the spot is safe based on what box it is located in
...
void getPuzzleBoard(int num[][10], bool noChange[]) { //get initial values of board from user
	int x;
	int y;
	int curSpace = 1;

	for (y = 1; y <= 9; ++y) {
		cout << "Enter numbers from row "<< y <<".\n";
		for (x = 1; x <= 9; ++x) {
			cin >> num[y][x];
			...
			if (num[y][x] > 0)
				...
				noChange[curSpace] = false; //if a number is entered, make that spot non-changeable
